...
final public class ActivityMain extends Activity {
    ...
    public static final String PKG = "com.android.liubz.androidtea";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        phase2(ctxt);
        Log.i(TAG, "onCreate: ActivityMain");
    }
    ...
    private void phase2(final Context ctxt) {
        ...
        // CheckNicDb
        try {
            int nicDbVer = prefs.getInt(Prefs.KEY_RESET_NICDB, Prefs.DEFAULT_RESET_NICDB);
            int curDbVer = getPackageManager().getPackageInfo(PKG, 0).versionCode;
            Log.i(TAG, "phase2: nicDbVer: " + nicDbVer + ", curDbVer: " + curDbVer);
            if (nicDbVer != curDbVer) {
                new DbUpdateNic();
            } else {
                // There is a NIC Db installed
                Log.i(TAG, "phase2: There is a NIC Db installed");
                phase3(ctxt);
            }
        } catch (NameNotFoundException e) {
            Log.e(TAG, "phase2: NameNotFoundException", e);
            phase3(ctxt);
        } catch (ClassCastException e) {
            Log.i(TAG, "phase2: ClassCastExceptionx", e);
            Editor edit = prefs.edit();
    ...
    private void phase3(final Context ctxt) {
        // Install Services DB
        try {
            int serviceDb = prefs.getInt(Prefs.KEY_RESET_SERVICESDB, Prefs.DEFAULT_RESET_SERVICESDB);
            int curVer = getPackageManager().getPackageInfo(PKG, 0).versionCode;
            Log.i(TAG, "phase3: serviceDb: " + serviceDb + ", curVer: " + curVer);
            if (serviceDb != curVer) {
                Log.i(TAG, "phase3: CreateServicesDb");
                new CreateServicesDb(ActivityMain.this).execute();
            } else {
                Log.i(TAG, "phase3: startDiscoverActivity");
                startDiscoverActivity(ctxt);
            }
        } catch (NameNotFoundException e) {
            Log.e(TAG, "phase3: NameNotFoundException", e);
            startDiscoverActivity(ctxt);
        }
    }

    private void startDiscoverActivity(final Context ctxt) {
        Log.i(TAG, "startDiscoverActivity: ");
