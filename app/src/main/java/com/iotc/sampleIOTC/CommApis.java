...
public class CommApis extends IOTCAPIs {
	private static final String TAG = "CommApis";
...
//										"m4.iotcplatform.com", "m5.iotcplatform.com");
			ms_nIOTCInit = IOTC_Initialize2(port);
			Log.i(TAG, "IOTC_Initialize(.)=" + ms_nIOTCInit + ", udpPort=" + port);
	...
	public void connect(String uid) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				int result = IOTC_Connect_ByUID2(uid, null, 0);
//				int result = IOTC_Connect_ByUID(uid);
	...
	public void mLanSearch()
	{

		String f = "ff" +  (char) (0x48);
		int[] nArray = new int[1];

		st_LanSearchInfo2[] ab_LanSearchInfo = IOTC_Lan_Search2(nArray,3000);
		Log.i(TAG, "mLanSearch --> st_LanSearchInfo: " + ab_LanSearchInfo);
		setLog(MainHandler.MSGTYPE_LOG, "mLanSearch --> st_LanSearchInfo: " + ab_LanSearchInfo);
		if (ab_LanSearchInfo != null) {
			setLog(MainHandler.MSGTYPE_LOG, "mLanSearch --> st_LanSearchInfo: " + Arrays.toString(ab_LanSearchInfo));
			Log.i(TAG, "mLanSearch --> st_LanSearchInfo: " + Arrays.toString(ab_LanSearchInfo));
		}
		...
		for(int i = 0; i < nArray[0];i++){
			...
			try 
			{
				...
				setLog(MainHandler.MSGTYPE_LOG, "DevName " + i +  " = " + new String(ab_LanSearchInfo[i].DeviceName,0, ab_LanSearchInfo[i].DeviceName.length,"utf-8"));
				Log.i(TAG, "mLanSearch: devname bytes: " + Arrays.toString(ab_LanSearchInfo[i].DeviceName));
				Log.i(TAG, "DevName " + i +  " = " + new String(ab_LanSearchInfo[i].DeviceName,0, ab_LanSearchInfo[i].DeviceName.length,"utf-8"));
			...
			if(nSID<0)
			{
				switch(nSID)
				{
					case IOTC_ER_NOT_INITIALIZED:
						str=String.format("Don'first call IOTC_Initialize() when connecting.(%d)", m_nSearchDev);
						break;
					...
					case IOTC_ER_FAIL_RESOLVE_HOSTNAME:
						str=String.format("Can'first resolved server's Domain name when connecting.(%d)", nSID);
						break;
					...
					case IOTC_ER_FAIL_GET_LOCAL_IP:
						str=String.format("Can'first Get local IP when connecting.(%d)", nSID);
						break;
